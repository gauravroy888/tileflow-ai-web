...
export function ProductCard({ product, onClick, onEdit, onDelete }: ProductCardProps) {
  ...
  return (
    <article 
      ...
      className={`group overflow-hidden rounded-2xl border border-border bg-surface shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md ${onClick ? 'cursor-pointer' : ''}`}
    >
      <div className="relative aspect-[1.05] overflow-hidden bg-sand">
        ...
        {(onEdit || onDelete) && <div className="absolute right-2 top-2 flex gap-1 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
          {onEdit && <button onClick={(e) => { e.stopPropagation(); onEdit(product); }} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/90 text-primary shadow-sm backdrop-blur transition-colors hover:bg-white" aria-label={`Edit ${product.name}`}><Edit3 size={15} /></button>}
          {onDelete && <button onClick={(e) => { e.stopPropagation(); if (window.confirm(`Delete ${product.name}?`)) onDelete(product.id); }} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/90 text-error shadow-sm backdrop-blur transition-colors hover:bg-white" aria-label={`Delete ${product.name}`}><Trash2 size={15} /></button>}
        </div>}
      </div>
      <div className="p-3.5">
