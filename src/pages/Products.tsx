...
import { Box, PackagePlus, Search, SlidersHorizontal } from 'lucide-react';
    ...
    <div className="page-shell mx-auto max-w-screen-xl space-y-5">
      <header className="flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-0.5 text-2xl font-extrabold tracking-tight">{t('products.title')}</h2>
          <p className="mt-1 text-sm text-textSecondary">Showroom samples, pricing and availability.</p>
        </div>
        <Button size="sm" className="shrink-0 gap-1.5" onClick={() => setIsAddModalOpen(true)}><PackagePlus size={17} /> Add item</Button>
      </header>

      <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
        <label className="relative block">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-textSecondary" size={19} />
          <input type="search" placeholder={t('products.search')} value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="h-12 w-full rounded-xl border border-border bg-surface pl-11 pr-4 text-sm shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15" />
        </label>
        <div className="flex items-center rounded-xl border border-[#F1D9CD] bg-accentSoft px-3 text-sm font-bold text-[#9A482A]">
          <SlidersHorizontal size={16} className="mr-2" /> {lowStockCount} low stock
        </div>
      </div>

      <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 sm:-mx-6 sm:px-6">
        {filters.map((filter) => <button key={filter.id} onClick={() => setActiveFilter(filter.id)} className={`shrink-0 rounded-full border px-3.5 py-2 text-xs font-extrabold transition-colors ${activeFilter === filter.id ? 'border-primary bg-primary text-white' : 'border-border bg-surface text-textSecondary hover:border-primary/30 hover:text-primary'}`}>{filter.label}</button>)}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-stone bg-surface px-6 py-14 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-sand text-primary"><Box size={22} /></div><h3 className="mt-4 font-extrabold">No products here yet</h3><p className="mx-auto mt-1 max-w-xs text-sm leading-5 text-textSecondary">Add your first tile or clear the filters to see your full catalogue.</p><Button className="mt-5 gap-2" onClick={() => setIsAddModalOpen(true)}><PackagePlus size={17} /> Add a product</Button></div>
      ) : (
